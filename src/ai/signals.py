...
class SignalGenerator:
    ...
    def _generate_signal(self, product_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result['mlp_prediction'] = predictions.get('mlp')
            result['gb_prediction'] = predictions.get('gb')
            result['ridge_prediction'] = predictions.get('ridge')
            for model_type in ['rf', 'lr', 'mlp', 'gb', 'ridge']:
                p = probas.get(model_type, [0.33, 0.34, 0.33])
                result[f'{model_type}_confidence'] = max(p) if isinstance(p, (list, tuple)) else 0.0
    ...
    def _get_features(self, product_id: str):
        ...
        # Phase 2: Increased from 7 to 14 days for more training data
