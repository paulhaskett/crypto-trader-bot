...
class ModelTrainer:
    ...
    def _find_best_atr_config(self, df: pd.DataFrame, X_train: np.ndarray, y_train: np.ndarray,
        ...
        for cfg in atr_configs:
            try:
                ...
                logger.debug(f"ATR config {cfg} failed: {e}")
                continue
        
        # If no valid config found, try fallback configs with relaxed criteria
        if best_config is None:
            for cfg in fallback_configs:
                try:
                    ...
                    if len(unique_classes) >= 3:
                        best_config = cfg
                        logger.info(f"Using fallback ATR config: {cfg}")
                        break
                except:
                    continue
    ...
    def _train_rf(self, product_id: str, X_train: np.ndarray, y_train: np.ndarray,
        ...
        """Train Random Forest model with class balancing for improved precision."""
        try:
            scaler = StandardScaler()
            X_train_scaled = scaler.fit_transform(X_train)
            X_test_scaled = scaler.transform(X_test)
            
            # Phase 2: Hyperparameter tuning with GridSearchCV (simplified for speed)
            ...
            try:
                ...
                logger.info(f"RF best params for {product_id}: {grid_search.best_params_}")
            except Exception as e:
                logger.warning(f"GridSearchCV failed for {product_id}, using default: {e}")
            ...
            n_important = sum(feature_importances > 0.01)  # Features with >1% importance
            top_indices = np.argsort(feature_importances)[-n_important:]
            
            logger.info(f"RF top features for {product_id}: {n_important} features with importance > 1%")
            logger.debug(f"  Top 5: {sorted(zip(top_indices[-5:], feature_importances[top_indices[-5:]]), key=lambda x: -x[1])}")
            ...
            logger.info(f"RF trained for {product_id}: win_rate={metrics.get('win_rate', 0):.1%}, "
    ...
    def _train_gb(self, product_id: str, X_train: np.ndarray, y_train: np.ndarray,
                  X_test: np.ndarray, y_test: np.ndarray, prices_test: np.ndarray) -> Dict:
        """Train Gradient Boosting model using HistGradientBoostingClassifier.
        
        HistGradientBoosting is ~10x faster than standard GradientBoosting and
        supports native class weight balancing for imbalanced datasets.
        """
        try:
            scaler = StandardScaler()
            X_train_scaled = scaler.fit_transform(X_train)
            ...
            # Phase 1: Use HistGradientBoostingClassifier (faster + class balancing)
            # This replaces the slower GradientBoostingClassifier
            model = HistGradientBoostingClassifier(
                max_iter=200,              # Increased from 150 equivalent
                max_depth=6,              # Same as before
                learning_rate=0.05,       # Reduced from 0.1 for better generalization
                l2_regularization=0.1,   # NEW - L2 regularization to prevent overfitting
                class_weight='balanced',  # NEW - handles class imbalance (unlike standard GB)
                random_state=42,
                early_stopping=True,      # NEW - prevents overfitting
                validation_fraction=0.1, # NEW - 10% for early stopping
