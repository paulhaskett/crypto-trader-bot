...
class TradingEngine:
    ...
    def _is_paper_trading(self) -> bool:
        """Check current trading mode from database (not cached)."""
        from src.database import db_manager
        return db_manager.get_paper_trading()
    ...
    def scan_for_signals(self) -> List[Dict[str, Any]]:
        ...
        # Sort candidates by confidence (highest first) - ensures best signals are chosen
        candidates.sort(key=lambda x: x['signal']['confidence'], reverse=True)
        print(f"SCAN_FOR_SIGNALS: Sorted candidates by confidence")
        for i, c in enumerate(candidates):
            print(f"  [{i+1}] {c['product_id']} {c['signal']['action']} ({c['signal']['confidence']:.1%})")
    ...
    def _evaluate_replacements(self, candidates: List[Dict]) -> List[Dict[str, Any]]:
        ...
        Rules:
        1. Only BUY signals can replace positions (need to own currency to sell)
        ...
        for candidate in candidates:
            ...
            # Only allow BUY signals for replacement (can't sell what we don't own)
            # SELL signals should only close existing positions through monitoring flow
            if action == 'SELL':
                print(f"REPLACE_SKIP: SELL {product_id} ({confidence:.1%}) - SELL signals not allowed for replacement", flush=True)
                continue
    ...
    def execute_signal(self, signal_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"EXECUTE_SIGNAL: Starting for {product_id} | Action: {signal['action']} | Confidence: {signal['confidence']:.1%}")

            # Get current prices for close price calculation
            current_prices = data_collector.get_current_prices()
            logger.info(f"EXECUTE_SIGNAL: Got current prices for {product_id}")
            ...
            if self.paper_trading:
                logger.info(f"EXECUTE_SIGNAL: Executing paper order for {product_id}")
                order_result = self._execute_paper_order(
                    product_id, side, size, entry_price, signal
                )
                logger.info(f"EXECUTE_SIGNAL: Paper order completed for {product_id}")
            else:
                logger.info(f"EXECUTE_SIGNAL: Executing LIVE order for {product_id}")
                order_result = self.execute_live_trade(
                    product_id, side, size
                )
                logger.info(f"EXECUTE_SIGNAL: Live order result for {product_id}: {order_result}")
            ...
            if order_result and order_result.get('success', False):
                ...
                position_details = {
                    ...
                    'trade_type': 'live' if not self._is_paper_trading() else 'paper'
                }

                self.active_positions[position_id] = position_details
                self.last_trade_time[product_id] = datetime.now()

                db_manager.save_open_position(position_details)
    ...
    def execute_live_trade(self, product_id: str, side: str, size: float, timeout: float = 30.0):
        """Execute a live trading order with timeout protection."""
        import time
        ...
        try:
            ...
            logger.info(f"Live order API result: {order_result} (took {elapsed:.1f}s)")

            if elapsed > timeout:
                logger.warning(f"Order took {elapsed:.1f}s (timeout: {timeout}s)")
            ...
            for position_id, position in list(self.active_positions.items()):
                try:
                    ...
                    if should_close:
                        ...
                        db_manager.update_open_position_price(position_id, current_price, pnl)
    ...
    def _close_position(self, position_id: str, pnl: float, reason: str, exit_price: float):
            ...
            if position_id in self.active_positions:
                ...
                db_manager.close_open_position(position_id, exit_price, pnl, reason)
    ...
    def switch_to_paper_trading(self):
        ...
        if self.paper_trading:
            logger.warning("Already in paper trading mode")
            return
        ...
        logger.info("Paper trading enabled - bot will no longer trade with real money")


    def run_trading_cycle(self, cycle_timeout: float = 300.0) -> Dict[str, Any]:
        """
        Run a complete trading cycle with heartbeat monitoring.

        Args:
            cycle_timeout: Maximum time in seconds for the cycle (default 5 minutes)
        ...
        print(f"Cycle timeout: {cycle_timeout}s", flush=True)

        def heartbeat(message: str):
            """Log heartbeat message with elapsed time."""
            elapsed = time_module.time() - cycle_start_time
            print(f"HEARTBEAT [{elapsed:.1f}s]: {message}", flush=True)
        ...
        try:
            ...
            if time_module.time() - cycle_start_time > cycle_timeout:
                logger.error("CYCLE TIMEOUT: Previous cycle exceeded timeout, skipping")
                return cycle_results

            heartbeat("Starting signal scan")
            signals = self.scan_for_signals()
            cycle_results['signals_found'] = len(signals)
            heartbeat(f"Signal scan complete: {len(signals)} signals found")

            # 2. Execute valid signals
            heartbeat(f"Executing {len(signals)} signals...")
            for i, signal_data in enumerate(signals):
                # Check timeout before each signal
                if time_module.time() - cycle_start_time > cycle_timeout:
                    logger.error(f"CYCLE TIMEOUT: Timeout exceeded during signal execution at signal {i+1}/{len(signals)}")
                    break

                product_id = signal_data.get('product_id', 'unknown')
                heartbeat(f"Executing signal {i+1}/{len(signals)}: {product_id}")

                result = self.execute_signal(signal_data)
                if result:
                    cycle_results['trades_executed'] += 1
                    heartbeat(f"Signal {product_id} executed successfully")

            heartbeat(f"All signals executed: {cycle_results['trades_executed']}/{len(signals)}")

            # 3. Monitor existing positions
            heartbeat("Starting position monitoring")
            closed_positions = self.monitor_positions()
            cycle_results['positions_closed'] = len(closed_positions)
            heartbeat(f"Position monitoring complete: {len(closed_positions)} closed")

            # Calculate total P&L from closed positions
            for closed in closed_positions:
                cycle_results['total_pnl'] += closed['pnl']

            cycle_results['cycle_time'] = time_module.time() - cycle_start_time
            heartbeat(f"Trading cycle completed in {cycle_results['cycle_time']:.1f}s: {cycle_results}")
            print("=" * 60, flush=True)

        except Exception as e:
            elapsed = time_module.time() - cycle_start_time
            print(f"ERROR in trading cycle after {elapsed:.1f}s: {e}", flush=True)
            import traceback
            traceback.print_exc()
            logger.error(f"Error in trading cycle after {elapsed:.1f}s: {e}")
