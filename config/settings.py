...
class Settings:
    ...
    MAX_SCALE_OUT_PER_POSITION: int = 3  # Max scale-out trades per position
    TAKE_PROFIT_LEVELS: list = [1.0, 2.0, 3.0]  # Default TP levels (%)
    ...
    def load_from_db(self):
        """Load persisted settings from database, overriding hardcoded defaults."""
        try:
            from src.database import db_manager

            # Mapping of DB setting keys → (settings attribute, converter)
            overrides = {
                'scale_in_enabled': ('SCALE_IN_ENABLED', lambda v: v.lower() == 'true'),
                'scale_out_enabled': ('SCALE_OUT_ENABLED', lambda v: v.lower() == 'true'),
                'market_check_interval': ('MARKET_CHECK_INTERVAL', int),
                'model_confidence_threshold': ('MODEL_CONFIDENCE_THRESHOLD', float),
                'stop_loss_min_percent': ('STOP_LOSS_MIN_PERCENT', float),
                'take_profit_level': ('TAKE_PROFIT_LEVELS', lambda v: [float(v), float(v) * 2, float(v) * 3]),
                'max_position_size': ('MAX_POSITION_SIZE', float),
                'max_concurrent_positions': ('MAX_CONCURRENT_POSITIONS', int),
                'display_currency': ('DISPLAY_CURRENCY', str),
                'trailing_stop_enabled': ('TRAILING_STOP_ENABLED', lambda v: v.lower() == 'true'),
                'trailing_stop_percent': ('TRAILING_STOP_PERCENT', float),
                'trailing_activation_buffer': ('TRAILING_ACTIVATION_BUFFER', float),
            }
            ...
            if loaded:
                import logging
                logging.getLogger(__name__).info(f"Loaded {loaded} settings from database")
